...
The notation SWEEP(1, 2, ..., K)[G] indicates successive applications of SWEEP(k)[G] with $k = 1, \dots, K$.
Sweeps on multiple positions do not need be carried out in any particular order.
...
- **Augment the original data** with a column of 1s on the left.
  ...
  In R, we can use the `cbind()` function to append a column of 1s to the left of X.
...
```
  ...
  Next, fit the multivariate linear model with the `lm()` R function:
...
```
  ...
  Given a $\Theta$ matrix, we can sweep on the positions of the independent variables and obtain the regression coefficients of the multivariate linear model with the variables we did not sweep over as dependent variables.
...
Let's index the rows of `H` with the names of the ivs (and the name of the intercept row) and the columns with the names of the dvs.
...
- SWEEP(1, $k_1, \dots, k_K$)[G] or as,
- SWEEP($k_1, \dots, k_K$)[SWEEP(1)[G]] or as,
- SWEEP($k_1, \dots, k_K$)[$\Theta$]

with $k_1, \dots, k_K$ being the positions of the $K$ predictors in matrix $G$.
