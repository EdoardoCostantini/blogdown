...
It performs elementary row operations on a $p \times p$ matrix which happen to be particularly useful to estimate multivariate linear models.
...
The notation indicating this transformation is usually a variation of \text{SWEEP}(k)[G]$, which can be read as sweeping matrix $G$ on row and column $k$.
The notation \text{SWEEP}(1, 2, ..., K)[G]$ indicates successive applications of $\text{SWEEP}(k)[G]$ with $k = 1, \dots, K$.
...
To obtain the estimates of the regression coefficients of a multivariate linear model, we need to sweep over the positions of the predictors the augmented covariance matrix of the data ($\Theta$).
